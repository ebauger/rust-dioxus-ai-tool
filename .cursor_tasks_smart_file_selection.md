...
*   **Story 2.1: Reading `.gitignore` Content**
    *   `[ ]` Task: Implement a function `read_gitignore_patterns(gitignore_file_path)` that takes the absolute path to a `.gitignore` file.
    *   `[ ]` Task: This function should read the entire content of the `.gitignore` file.
    *   `[ ]` Task: It should split the content into individual lines.
    *   `[ ]` Task: Handle potential file I/O errors gracefully (e.g., if the file, previously detected, becomes unreadable).
    *   `[ ]` Task: Return a list of raw string lines from the file.

*   **Story 2.2: Pre-processing `.gitignore` Lines**
    *   `[ ]` Task: Implement a function `preprocess_gitignore_lines(raw_lines)` that takes the list of raw lines from `read_gitignore_patterns`.
    *   `[ ]` Task: For each line, remove leading and trailing whitespace.
    *   `[ ]` Task: Filter out (ignore) any line that is empty after trimming whitespace.
    *   `[ ]` Task: Filter out (ignore) any line that starts with a `#` character (comments).
    *   `[ ]` Task: Return a new list containing only the processed, effective pattern strings.

*   **Story 2.3: Implementing `.gitignore` Pattern Matching Logic**
    *   *Note: This is the most complex part. Consider if a well-tested third-party library for `.gitignore` pattern matching is available and suitable for the project's language/environment. If implementing from scratch, extreme care is needed.*
    *   `[ ]` Task: Research the full `.gitignore` pattern specification, including:
        *   Literal matching (`file.txt`).
        ...
        *   Patterns starting with `/` (anchor to the workspace root).
        *   Patterns containing `/` (path-specific relative to `.gitignore` location).
        *   Negation patterns starting with `!` (un-ignore a file).
    *   `[ ]` Task: Implement a core function `is_file_ignored(relative_file_path, processed_patterns, workspace_root)` that takes a file's relative path, the list of preprocessed `.gitignore` patterns, and the workspace root.
    *   `[ ]` Task: Inside `is_file_ignored`:
        *   `[ ]` Task: Iterate through the `processed_patterns` *in order*. The last matching pattern for a given path determines its status (with `!` negating).
        *   `[ ]` Task: For each pattern, convert it into a regular expression or use a glob-matching utility suitable for `.gitignore` syntax.
            *   `[ ]` Task: Ensure `*` does not cross directory boundaries unless `**` is used.
            *   `[ ]` Task: Correctly handle anchoring for patterns starting with `/`.
            *   `[ ]` Task: Correctly handle directory-only patterns (ending with `/`). This means the pattern `build/` should match `build/file.txt` and `build/subdir/another.txt`.
        *   `[ ]` Task: Track if a path is matched by any ignore pattern and separately if it's matched by a negation pattern. The final decision for a path: it is ignored if the last relevant pattern matching it is an ignore pattern; it is not ignored if the last relevant pattern matching it is a negation pattern.
    *   `[ ]` Task: Return `true` from `is_file_ignored` if the file should be excluded based on the patterns, `false` otherwise.

*   **Story 2.4: Unit Testing for Pattern Matching**
    *   `[ ]` Task: Create a comprehensive test suite for `is_file_ignored`.
    *   `[ ]` Test Case: Simple file match (`config.ini` matches `config.ini`).
    *   `[ ]` Test Case: Simple directory match (`logs/` matches `logs/error.log` and `logs/access/something.log`).
    *   `[ ]` Test Case: Wildcard match (`*.tmp` matches `data.tmp` and `src/temp.tmp`).
    *   `[ ]` Test Case: Globstar match (`foo/**/bar.txt` matches `foo/bar.txt` and `foo/a/b/bar.txt`).
    *   `[ ]` Test Case: Anchored match (`/root.file` matches `root.file` but not `src/root.file`).
    *   `[ ]` Test Case: Path-specific match (`docs/README.md` matches `docs/README.md` but not `README.md`).
    *   `[ ]` Test Case: Negation (`!important.md`, with `*.md` also present).
    *   `[ ]` Test Case: Order of precedence (e.g. `*.log` then `!debug.log` then `important/debug.log` for path `important/debug.log`).
    *   `[ ]` Test Case: Patterns with leading/trailing spaces (should have been trimmed and match correctly).
    *   `[ ]` Test Case: Files within an ignored directory (e.g. if `build/` is ignored, `build/app.exe` should be ignored).
    *   `[ ]` Test Case: Negated file inside an ignored directory (e.g. `build/`, `!build/special.dll`).
...
*   **Story 3.1: Orchestrating the Selection Logic**
    *   `[ ]` Task: In the main "workspace opened" handler (from Story 1.1), after getting the workspace root:
        *   `[ ]` Task: Call `check_for_gitignore(workspace_root)`.
        *   `[ ]` Task: Call `get_all_workspace_files(workspace_root)` to get the list of all relative file paths.
        *   `[ ]` Task: Initialize a data structure (e.g., a Set or Map) to store the paths of selected files. Let's call it `final_selected_files`.
    *   `[ ]` Task: If `check_for_gitignore` returned a path (meaning `.gitignore` exists):
        *   `[ ]` Task: Call `read_gitignore_patterns` with the path.
        *   `[ ]` Task: Call `preprocess_gitignore_lines` with the raw patterns.
        *   `[ ]` Task: For each `file_path` in the `all_workspace_files` list:
            *   `[ ]` Task: Call `is_file_ignored(file_path, processed_patterns, workspace_root)`.
            *   `[ ]` Task: If `is_file_ignored` returns `false` (file is NOT ignored), add `file_path` to `final_selected_files`.
    *   `[ ]` Task: If `check_for_gitignore` returned `null` (no `.gitignore`):
        *   `[ ]` Task: Ensure `final_selected_files` remains empty (or is explicitly cleared), effectively unselecting all files.
