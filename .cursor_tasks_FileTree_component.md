...
*   [ ] **Task 2.1:** Create a helper function `build_tree_from_file_info(files: &[FileInfo], selected_paths: &HashSet<PathBuf>) -> Vec<FileTreeNode>` in `src/components/file_tree.rs`.
*   [ ] **Task 2.2:** Implement logic within `build_tree_from_file_info` to parse `PathBuf` from `FileInfo` and create intermediate folder nodes as needed.
...
*   [ ] **Task 2.3:** Handle edge case: empty `files` input.
*   [ ] **Task 2.4:** Write unit tests in `src/components/file_tree_test.rs` for `build_tree_from_file_info`:
...
*   [ ] **Task 3.1:** Define `FileTreeProps` struct in `src/components/file_tree.rs`.
...
*   [ ] **Task 3.2:** Create the `FileTree` component function `pub fn FileTree(props: FileTreeProps) -> Element` in `src/components/file_tree.rs`.
*   [ ] **Task 3.3:** Inside `FileTree`, use a `use_signal` to store the constructed `Vec<FileTreeNode>` (e.g., `tree_nodes`).
*   [ ] **Task 3.4:** Use `use_effect` to call `build_tree_from_file_info` when `props.all_files` or `props.selected_paths` changes and update the `tree_nodes` signal.
*   [ ] **Task 3.5:** Implement initial RSX to iterate over the top-level `tree_nodes` and render a basic representation (name and type, checkbox).
*   [ ] **Task 3.6:** Create `src/components/file_tree_test.rs` if it doesn't exist.
*   [ ] **Task 3.7:** Add `mod file_tree;` and `mod file_tree_test;` to `src/components/mod.rs`.
*   [ ] **Task 3.8:** Add `pub use file_tree::FileTree;` to `src/components/mod.rs`.
...
*   [ ] **Task 4.1:** Define a new Dioxus component `FileTreeNodeDisplay(node: Signal<FileTreeNode>, selected_paths: Signal<HashSet<PathBuf>>)` within `src/components/file_tree.rs`.
    *   Alternatively, a recursive `rsx!` macro call within `FileTree` itself. A sub-component is often cleaner.
*   [ ] **Task 4.2:** In `FileTreeNodeDisplay`, render the node's checkbox, icon (folder/file), and name.
    *   Folder icon should change based on `node.read().is_expanded.read()`. (e.g., "📂" for expanded, "📁" for collapsed).
    *   File icon: "📄".
*   [ ] **Task 4.3:** Apply indentation based on `node.read().depth` (e.g., `style: "padding-left: {node.read().depth * 20}px;"`).
*   [ ] **Task 4.4:** If `node.read().node_type == TreeNodeType::Folder` and `node.read().is_expanded.read()` is true, recursively call `FileTreeNodeDisplay` for each child in `node.read().children`.
*   [ ] **Task 4.5:** Update `FileTree` component's RSX to use `FileTreeNodeDisplay` for its top-level nodes.
...
*   [ ] **Task 5.1:** In `FileTreeNodeDisplay`, add an `onclick` handler to the row or folder icon of a folder node.
*   [ ] **Task 5.2:** The `onclick` handler should toggle the `node.write().is_expanded.toggle()` value.
*   [ ] **Task 5.3:** Ensure clicking a file row does not attempt to expand/collapse.
...
*   [ ] **Task 6.1:** In `FileTreeNodeDisplay`, bind the checkbox's `checked` attribute.
    *   `checked: node.read().selection_state.read() == NodeSelectionState::Selected`
*   [ ] **Task 6.2:** Bind the checkbox's `indeterminate` attribute.
    *   `indeterminate: node.read().selection_state.read() == NodeSelectionState::PartiallySelected`
*   [ ] **Task 6.3:** Modify `build_tree_from_file_info` or a subsequent processing step to correctly initialize `selection_state` for each node based on `selected_paths` and children's states. This involves:
    *   A file node is `Selected` if its path is in `selected_paths`, else `NotSelected`.
    *   A folder node's state is calculated (see Story 7).
...
*   [ ] **Task 7.1:** Create a helper function `set_node_and_descendants_state(node: &mut FileTreeNode, new_state: bool, selected_paths_writer: &mut WriteSignal<HashSet<PathBuf>>)` in `src/components/file_tree.rs`.
    *   If `new_state` is true, add `node.path` to `selected_paths_writer` and set `node.selection_state` to `Selected`.
    *   If `new_state` is false, remove `node.path` from `selected_paths_writer` and set `node.selection_state` to `NotSelected`.
    *   Recursively call for children. For folder children, their `selection_state` also becomes `Selected` or `NotSelected`.
*   [ ] **Task 7.2:** In `FileTreeNodeDisplay`, add an `oninput` event handler to the checkbox.
*   [ ] **Task 7.3:** The `oninput` handler should:
...
*   [ ] **Task 7.3 (Revised):** The checkbox `oninput` handler:
...
*   [ ] **Task 8.1:** Create a helper function `calculate_folder_selection_state(folder_node: &FileTreeNode) -> NodeSelectionState` in `src/components/file_tree.rs`.
